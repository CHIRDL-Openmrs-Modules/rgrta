...
public class Calculator
{
	...
	private Percentile lookupPercentile(String type,double agemos,int sex)
	{
		...
		{
			percentileTable = RgrtaService.getBmiage(agemos, sex);
		}

		if (type.equalsIgnoreCase(PERCENTILE_WT))
		{
			percentileTable = RgrtaService.getWtageinf(agemos, sex);
		}

		if (type.equalsIgnoreCase(PERCENTILE_LEN))
		{
			percentileTable = RgrtaService.getLenageinf(agemos, sex);
		}

		if (type.equalsIgnoreCase(PERCENTILE_HC))
		{
			percentileTable = RgrtaService.getHcageinf(agemos, sex);
		}
