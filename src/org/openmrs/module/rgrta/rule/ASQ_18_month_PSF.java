/********************************************************************
 Translated from - ASQ_18_month_PSF.mlm on Tue Jul 20 11:24:45 EDT 2010
...
public class ASQ_18_month_PSF implements Rule, DssRule{
	...
	public String getData(){
		return "read read read read read read read If read endif";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result asq=context.read(
				patient,context.getLogicDataSource("CHICA"),
				new LogicCriteria("ASQ_18_month").last());
			resultLookup.put("asq",asq);
			Result devref=context.read(
				patient,context.getLogicDataSource("CHICA"),
				new LogicCriteria("Developmental_Referral").last());
			resultLookup.put("devref",devref);}
			...
			if(evaluate_logic(parameters)){
				...
				int varLen = 0;
				varLen = "ASQ_JIT".length();
				value=userVarMap.get("ASQ_JIT");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("ASQ_JIT".endsWith("_value"))
				{
					variable = "ASQ_JIT".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("ASQ_JIT".endsWith("_date"))
				{
					variable = "ASQ_JIT".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("ASQ_JIT") != null){
						value = resultLookup.get("ASQ_JIT").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","ASQ_JIT");
				}
				varLen = "firstname".length();
				value=userVarMap.get("firstname");
				if(value != null){
					parameters.put("param2",value);
				}
				// It must be a result value or date
				else if("firstname".endsWith("_value"))
				{
					variable = "firstname".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("firstname".endsWith("_date"))
				{
					variable = "firstname".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).getResultDate().toString();
					}
				}
				...
				{
					if (resultLookup.get("firstname") != null){
						value = resultLookup.get("firstname").toString();
					}
				}
				if(value != null){
					parameters.put("param2",value);
				...
				{
					parameters.put("param2","firstname");
				}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result devref = (Result) resultLookup.get("devref");
		Result asq = (Result) resultLookup.get("asq");
		...
		if((!asq.isNull()&&asq.toString().equalsIgnoreCase("passed"))||
			(!asq.isNull()&&asq.toString().equalsIgnoreCase("failed"))||
			(!devref.isNull()&&devref.toString().equalsIgnoreCase("First_steps"))){
			return false;
		}
		else{
			return true;
